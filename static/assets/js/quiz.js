(function($) {
    $.fn.jquizzy = function(settings) {
        ...
        var defaults = {
            data: null,
            startImg: '/static/assets/img/start.jpg',
            endText: '已结束!',
            shortURL: null,
            sendResultsURL: null,
            resultComments: {
        ...
        for (questionsIteratorIndex = 0; questionsIteratorIndex < config.data.length; questionsIteratorIndex++) {
			   contentFob += '<div class="slide-container"><div class="question-number">' + (questionsIteratorIndex + 1) + '/' + config.data.length + '</div><div class="question">' + config.data[questionsIteratorIndex].questioncontent + '</div><ul class="answers">';
			
			   if(config.data[questionsIteratorIndex].answers==null){
				contentFob +='<div class="am-form-group"> <textarea class="" rows="5" id="doc-ta-1"></textarea> </div>'
			}else{
			  var str = config.data[questionsIteratorIndex].answers.substring(0);
			  var answerss= str.split('`');
			 for (answersIteratorIndex = 0; answersIteratorIndex < answerss.length; answersIteratorIndex++) {
              
                contentFob += '<li>' + answerss[answersIteratorIndex] + '</li>';
            }
           }
            ...
            if (questionsIteratorIndex < config.data.length - 1) {
            ...
            contentFob += '</div></div>';
            answers.push(config.data[questionsIteratorIndex].correctAnswer);
        }
        ...
        var progress = superContainer.find('.progress'),
            ...
            userAnswers = [],
            questionLength = config.data.length,
        ...
        superContainer.find('.final').click(function() {
            ...
            for (var i = 0,
                ...
                resultSet += '<div class="resultsview-qhover">' + config.data[i].data;
                resultSet += "<ul>";
                for (answersIteratorIndex = 0; answersIteratorIndex < answerss.length; answersIteratorIndex++) {
                    var classestoAdd = '';
                    if (config.data[i].correctAnswer == answersIteratorIndex + 1) {
                        classestoAdd += 'right';
                    }
                    if (userAnswers[i] == answersIteratorIndex + 1) {
                        classestoAdd += ' selected';
                    }
                    resultSet += '<li class="' + classestoAdd + '">' + answerss[answersIteratorIndex] + '</li>';
                }
